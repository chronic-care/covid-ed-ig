...
2. Run `./_updateValueSet.sh`. This updates the CQF tooling, generates the new valueset, copies it to the `./input/cql` folder and then updates `healthCodes.js` with the new codes as well.
...
Various CQL helper definitions, such as `MostRecent` and `FindDate`, exist in files like [CDSConnectCommons.cql](/input/cql/CDSConnectCommons.cql) and [FHIRHelpers.cql](/input/cql/FHIRHelpers.cql).
